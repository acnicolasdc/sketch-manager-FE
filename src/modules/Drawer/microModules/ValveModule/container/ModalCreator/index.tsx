...
import { CIRCLE, OPTIONS_TYPES_MATERIAL, OPTIONS_TYPES_PRESSURE, OPTIONS_TYPES_SIZE } from '../../../../utils/assets';
...
const ModalCreator: React.FunctionComponent<ModalCreatorProps> = ({ cancell=FAKE_FN, isOpen= false }) => {
  const { material: pfmaterial, pressure: pfPressure, size: pfSize } = useSelector((state: any) => state.get('pipeForm').toObject());
  const [{ pressure, sizev, size, materialv, material, value}, dispatch] = useReducer(reducer, initialState);
  ...
  return (
      <Modal onClose={cancell} isOpen={isOpen} unstable_ModalBackdropScroll={true}>
        ...
        <Select
              ...
              onChange={(params:any) => dispatch({ type:ReducerValve.setPressure , value:params.value})}
              overrides={{
                Root:{
                  style:{
                    marginBottom:'10px'
        ...
        />
        <HeadingLevel><Heading styleLevel={6}
        overrides={{
          Block:{
            style:{
              marginBottom:'5px'
        ...
        >Valve</Heading></HeadingLevel>
        <Row>
        <Select
              options={OPTIONS_TYPES_SIZE}
              value={sizev}
              placeholder="Select valve size"
              onChange={(params:any) => dispatch({ type:ReducerValve.setSizev , value:params.value})}
        />
          <Select
              options={OPTIONS_TYPES_MATERIAL}
              value={materialv}
              placeholder="Select valve material"
              onChange={(params:any) => dispatch({ type:ReducerValve.setMaterialv , value:params.value})}
          />
        </Row>
        <HeadingLevel><Heading styleLevel={6}
            overrides={{
        ...
        >Main</Heading></HeadingLevel>
        <Row>
        <Select
              options={OPTIONS_TYPES_SIZE}
              value={size}
              placeholder="Select main size"
              onChange={(params:any) => dispatch({ type:ReducerValve.setSize , value:params.value})}
          />
        <Select
              options={OPTIONS_TYPES_MATERIAL}
              value={material}
              placeholder="Select main material"
              onChange={(params:any) => dispatch({ type:ReducerValve.setMaterial , value:params.value})}
        />
        </Row>
        <HeadingLevel><Heading styleLevel={6}>Valve code</Heading>
      </HeadingLevel>
        <PinCode
          values={value}
          onChange={({values}) => dispatch({ type:ReducerValve.setValue , value:values})}
        />
        </ModalBody>
        <ModalFooter>
          <ModalButton
